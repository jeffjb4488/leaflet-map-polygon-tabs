...
$.getJSON("data/polygon_test.geojson",function(json){
// add GeoJSON layer to the map once the file is loaded
    geoJsonLayer = L.Proj.geoJson(json ,{
	    style: style,
...
function style(feature) {
  return {
    fillColor: getColor(feature.properties["Lot_Number" + time]),
    weight: 1,
    opacity: 1,
    color: 'black',
    fillOpacity: 0.9
...
info.update = function (props) {
  ...
  this._div.innerHTML = (props ?
    '<div class="areaName">' + props.Lot_Number + '</div>' : '<div class="areaName faded"><small>Hover over areas<br>Click tabs or arrow keys</small></div>') + '<div class="areaLabel"><div class="areaValue">Home Value Index</div>' +(props ? '' + (checkNull(props["Lot_Number" + time])) : '--') + '</div>';
};
info.addTo(map);
